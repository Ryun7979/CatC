...
public class MbVoiceWrapper : MonobitEngine.VoiceChat.MonobitVoice
{
	...
	/// ボイスデータ再生後処理
	/// </summary>
	public override void OnEndDecode()
	{
		base.OnEndDecode();
	}
            ...
            case StreamType.BROADCAST:
                {
                    ...
                    // ボイスチャットデータの送信処理
                    MonobitNetwork.SendVoice(monobitView, target, new int[0], header, voice);
                }
                break;
            case StreamType.MULTICAST:
                {
                    ...
                    {
                        MonobitEngineBase.MonobitNetwork.SendVoice(monobitView, MonobitTargets.LimitedPlayer, multicastPlayerList.ToArray(), header, voice);
                    }
                }
                break;
        }
